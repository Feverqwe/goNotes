...
- Архив (`is_archived`) и корзина (`is_deleted`) — разные состояния. Поиск
  глобальный по обычным и архивным заметкам; в корзине он ограничен корзиной.
  Тег работает как пользовательская категория, внутри которой можно отдельно
  показать текущие или архивные заметки. Корзина может содержать заметки из
  обоих состояний.
...
- `notes-ui/src/components/NotesHeader/` — поиск, сброс фильтров и переключение
  между текущими и архивными заметками глобально или внутри одной категории.
- `notes-ui/src/components/NavigationDrawer/` — постоянная desktop- или
  выдвижная mobile-навигация, создание заметки, корзина и выбор темы.
- `notes-ui/src/components/TagsNavigation/` и `TagsNavigationList/` — категории,
  их выбор и ручная сортировка; отдельного пункта архива здесь нет.
...
корзине остается внутри корзины. Архив переключается в `NotesHeader`: без тега
это глобальный архив, с одной выбранной категорией — архив этой категории.
