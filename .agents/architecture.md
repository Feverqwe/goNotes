...
- Архив (`is_archived`) и корзина (`is_deleted`) — разные состояния. Поиск
  ...
  обоих состояний.
- Delete — двухэтапная операция: активная заметка сначала попадает в корзину, заметка, уже находящаяся в корзине, удаляется из БД вместе с физическими файлами. Restore сбрасывает `is_deleted`.
