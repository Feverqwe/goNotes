...
- При добавлении нового HTTP API или изменении доступных через API возможностей
  заметок синхронно обновляй MCP-контракт в `internal/mcp.go`: инструменты,
  входные и выходные DTO, описания и тесты. При появлении новой кастомной
  Markdown-разметки также добавляй правила ее использования и сохранения в
  инструкции MCP-сервера, чтобы агенты не теряли разметку при редактировании.
